...
class FreqStyle(HandleCodeRepo):
	...
	def node_selection(self, tree, features):
	...
	def __call__(self) -> typing.Tuple[typing.List[str]]:
		...
		for tree in trees:
			statements.append(self.__len__([True for _, node in tree if node.__class__.__name__ in self.stmt_features]))
			expressions.append(self.__len__([True for _, node in tree if node.__class__.__name__ in self.expr_features]))
			controls.append(self.__len__([True for _, node in tree if node.__class__.__name__ in self.ctrl_features]))
		numstat_stmts, numstat_expr, numstat_ctrls = ([] for _ in range(3)) 
		for source_file, stmt_feature, expr_feature in zip(source_files, statements, expressions):
			self._FE(source_file, stmt_feature, numstat_stmts)
			self._FE(source_file, expr_feature, numstat_expr)


		return numstat_expr[:10]
